...
public class LinkedListTester
{
   public static void main(String[] args)
   {
      ...
      try
      {
         // build a list, get an iterator, call next(), call list.removeFirst(), call next() again
         lst = new LinkedList();
         lst.addLast("A");
         lst.addLast("B");
         lst.addLast("C");
         ...
         assertEquals("A", iter.next());
         lst.removeFirst();
         check("BCDE", lst, "Remove last element");
         iter.next();
         System.out.println("FAIL: no exception");
      }
      catch (ConcurrentModificationException e)
      {
         System.out.println("Passed \"stale iterator detected\".");
      }
   }
