...
In a paint program, a “flood fill” fills all empty pixels of a drawing with a given color,
...
If it has not yet been filled, fill the corresponding array location with a number 1, 2, 3, 
    and so on (to show the order in which the square is filled).
